...
class autoptimizeCache {
    ...
    public function __construct($md5,$ext='php') {
        $this->cachedir = apply_filters( 'autoptimize_filter_cache_cachedir', AUTOPTIMIZE_CACHE_DIR );
    ...
    static function clearall() {
        ...
        foreach (array("","js","css") as $scandirName) {
            $scan[$scandirName] = scandir($this->cachedir.$scandirName);
        }
        
        // clear the cachedirs
        foreach ($scan as $scandirName=>$scanneddir) {
            $thisAoCacheDir=rtrim($this->cachedir.$scandirName,"/")."/";
    ...
    static function cacheavail() {
        ...
        foreach (array("","js","css") as $checkDir) {
            if(!autoptimizeCache::checkCacheDir($this->cachedir.$checkDir)) {
                return false;
            }
        }
